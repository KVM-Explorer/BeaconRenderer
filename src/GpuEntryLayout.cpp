...
ComPtr<ID3D12RootSignature> GpuEntryLayout::CreateRenderSignature(ID3D12Device *device, uint gbufferNum)
{
    ...
    std::array<CD3DX12_DESCRIPTOR_RANGE, 1> cubeRange = {};
    cubeRange.at(0).Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0, 3); // CubeMap
    ...
    rootParameters.at(7).InitAsDescriptorTable(cubeRange.size(),cubeRange.data()); // CubeMap
